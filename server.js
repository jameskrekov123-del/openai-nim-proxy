...
    // --- GLM PARAGRAPH & THINKING FIX ---
    ...
    const nimRequest = {
      ...
      stream: stream || false,
      // Exact structure from your NVIDIA screenshot
      extra_body: {
        chat_template_kwargs: { 
          thinking: ENABLE_THINKING_MODE,
          ...(isGLM && { enable_thinking: true }), // Required specifically for GLM thinking
  ...
  if (stream) {
      ...
      response.data.on('data', (chunk) => {
        ...
        buffer = lines.pop() || ''; // Essential: holds partial JSON strings so they don't crash
        ...
        for (let line of lines) {
          ...
          try {
            ...
            if (data.choices?.[0]?.delta) {
              ...
              // Clean meta-tokens that plague NVIDIA NIM outputs
            ...
            // Buffer handled the split, so this only catches actual corrupted data
          }
        }
      });
      ...
      // Cleanup on client disconnect to prevent Render "Ghost Processes"
      req.on('close', () => {
